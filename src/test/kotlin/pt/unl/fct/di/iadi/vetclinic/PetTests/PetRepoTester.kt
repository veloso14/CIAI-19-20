...
class PetRepoTester {
    ...
    companion object Constants {
        ...
        val cobradiezmos = PetDAO(-1L, "cobra", "Dog", false, emptyList(), dono)
        val arrojado = PetDAO(-1L, "arrroja", "Cat",true, emptyList(), dono)
...
@Transactional
@Test
fun ` test on findAll without frozen pets`() {

    pets.deleteAll()
    clients.save(dono)
    val pet0 = pets.save(cobradiezmos)
    assertThat(pet0.id, not(equalTo(cobradiezmos.id)))//the id is different because it is generated by Spring
    assertThat(pet0.name, equalTo(cobradiezmos.name))
    assertThat(pet0.species, equalTo(cobradiezmos.species))
    assertThat(pet0.frozen, equalTo(cobradiezmos.frozen))

    assertThat(pets.findAllByFrozenFalse().toList(), equalTo(listOf(pet0)))

    val pet1 = pets.save(arrojado)
    assertThat(pet1.id, not(equalTo(arrojado.id)))//the id is different because it is generated by Spring
    assertThat(pet1.name, equalTo(arrojado.name))
    assertThat(pet1.species, equalTo(arrojado.species))
    assertThat(pet1.frozen, equalTo(arrojado.frozen))

    assertThat(pets.findAllByFrozenFalse().toList(), equalTo(listOf(pet0)))

    pets.delete(pet0)

    assertThat(pets.findAllByFrozenFalse().toList(), equalTo(emptyList()))
